...
So the [JDK 21](https://openjdk.org/projects/jdk/21/) came out, we already had some insight how the virtual threads looked like earlier with the _preview_ flag. However with a proper release now it's more accessible and I encourage everyone that can to upgrade to this version. 
<!-- more -->
...
Virtual threads are lightweight threads that are cheap to create and almost infinitely plentiful. That means that they are an implementation of threads that is provided by the JDK rather than the operating system (OS). 
...
So what are virtual threads really and how does it differ from platform threads?

As you read in the introduction, platform threads acts as a wrapper around the operating system (OS) threads. The JDK's virtual thread scheduler assigns the virtual thread for execution on a platform thread by mounting the virtual thread on a platform thread. In other words, the platform thread is basically the carrier of the virtual thread.

Virtual threads is not by any mean faster threads. They do not run any code faster than platform threads. They do however provide scale (higher throughput) but not speed (lower latency).
...
We will implement a task processing system using platform threads to establish a performance baseline. Afterward, we will create an equivalent system using virtual threads and compare the throughput of both implementations to evaluate the benefits of virtual threads.
...
Exception handling is used to address potential interruptions during this wait. Once the waiting period concludes, the _producer_ adds the new task to the buffer and logs a message indicating successful task production. A notification is sent to awaken other waiting threads, such as consumers, signaling the availability of new tasks. 
...
As you clearly can see, virtual threads is the winner here for running 100,000 tasks with a throughput more than double than for platform threads. 

#### Platform Threads

**Performance Metrics:**\
**Elapsed Time:** 1.068746208 seconds\
**Total Tasks Processed:** 100000\
**Throughput (Tasks/Second):** 93567.5834463405


#### Virtual Threads

**Performance Metrics:**\
**Elapsed Time:** 0.465984125 seconds\
**Total Tasks Processed:** 100000\
**Throughput (Tasks/Second):** 214599.5853399512
...
The platform thread to which the scheduler assigns a virtual thread is called the virtual threads's carrier. Virtual threads can be scheduled on different carriers over the course of its lifetime. Basically, the scheduler does nto maintain affinity between a virtual thread and any kind of platform thread.

The scheduler does not at this time implement time sharing for virtual threads. While time sharing can be an effective way of reducing latency of certain tasks where it could be that there are a small number of platform threads and CPU utilization is at 100%, it is not clear that it would be as effective with a million virtual threads.

## Thread-per-request strategy

To handle concurrent user requests that are independent of each other, server applications generally have been using thread-pre-request strategy so that it can handle a request dedicated to a thread under its entire duration.

The strategy is easy to understand, easy to implement, debug and profile because it's using the platform's unit of concurrency to represent the application's unit of concurrency.

Unfortunately, since the number of available threads  is limited because how the JDK implements threads as wrappers around the operating system (OS) threads. They are costly, so using too many of them would make the implementation ill-suited because it's a limiting factor if we want to focus on throughput before we exhaust CPU or network connections.
...
## Thread pools - why you should not use it
...
Migrating the application code using the virtual-thread-per-task `ExercutorService` similar to the tradtional thread-pool based `ExercutorService` is not neccessary since there is never a need to pool them. A thread pool is like any other resource pool, intended to share expensive resources, but with virtual threads, they are not expensive.

We typically use thread pools to limit concurrent access to limited resources. Let's say a service cannot handle more than 20 concurrent requests, we typically make all request to the service via tasks submitted to a thread pool of size 20 and that will fix it. The high cost of platform threads has made thread pools ubiquitous.

Avoid using pool for virtual threads!

## Memory use and interaction with GC

Java's garbage-collected heap stores the stacks of virtual threads as chunk objects. It will grow and shrink the stacks as the application runs, both to be memory-efficient and to accomodate stacks of depth up to the JVM's configured platform thread stack size.

The amount of activity of the heap space and garbage collector that virtual threads require is hard, specifically when we compare that to asynchronous code. If we would use a million virtual threads, that would be equal to at least a million objects, but so do a million tasks sharing a pool of platform threads.
...
So now you probably a little bit more about Virtual Threads. The tests I conducted could probably be improved, so please submit a PR to my repo (link below). I will update the article if neccessary!  
